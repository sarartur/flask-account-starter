...
from ..user.enums import AccountLogActions, AccountBlockReasons
from .. import email
...
def handle_login(user, passwd, remember):
    ...
    if user.is_blocked:
        ...
        return 1
    if (
        request.remote_addr not in user.verified_ips
...
def _handle_invalid_credentials(user):
    ...
    last_login_or_passwd_reset = (
        UserAccountLog.query.filter_by(user_id=user.id)
        .filter(
            UserAccountLog.action
            in [AccountLogActions.LOGIN_SUCCESS, AccountLogActions.PASSWORD_RESET]
        )
        .order_by(desc(UserAccountLog.timestamp))
        .first()
    )
    failed_attempts_since_query = UserAccountLog.query.filter_by(
        user_id=user.id
    ).filter_by(action=AccountLogActions.LOGIN_FAILURE)
    if last_login_or_passwd_reset:
        failed_attempts_since_query = failed_attempts_since_query.filter(
            UserAccountLog.timestamp > last_login_or_passwd_reset.timestamp
        )
    failed_attempts_since = failed_attempts_since_query.count()
    if failed_attempts_since >= current_app.config["LOGIN_MAX_RETIRES"]:
        user.update(
            is_blocked=True, block_reason=AccountBlockReasons.LOGIN_ATTEMPTS_EXCEEDED
        )
