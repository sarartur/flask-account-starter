...
from .. import email
from ..auth import login_required, verification_setting_required
...
def profile():
    login_records = (
        UserAccountLog.query.filter_by(user_id=current_user.id)
        .order_by(desc(UserAccountLog.timestamp))
        .paginate(request.args.get("page", 1, type=int), 25)
    )
    return render_template(
        "user/profile.html", user=current_user, login_records=login_records
    )
...
def profile_not_verified():
    ...
    return render_template(
        "user/profile_not_verified.html", user=current_user, form=form
    )
